...
class TextInput extends React.Component<Props>{ 
    ...
    render(){ 
        const {onChangeText,onBlur,value, clear,placeholder,error,label,password} = this.props

        return (  
            <View key={label} style={{height :  60, width : "100%",marginBottom : 4}}>
              <View style={[styles.textAreaStyles,styles.addressInputWrapper]} >               
                      <RnTextInput {...this.props} value={value} onBlur={onBlur}  onChangeText={onChangeText}  secureTextEntry={password}  placeholder={label}   style={styles.addressInput} />  
