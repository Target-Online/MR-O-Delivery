...
const ShoppingBasket = ({ isWhite, style, navigation }) => (
...
const Header = props => {
  const [currentRequest] = React.useContext(SessionContext)
  ...
  const renderRight = () => {
    ...
    switch (title) {
      case 'Request Details':
        return currentRequest.isShopping && ([
          <ShoppingBasket navigation={navigation} isWhite={white}/>
        ]);
      case 'Categories':
        return ([
  ...
  const renderHeader = () => {
    ...
    return (
      <Block style={headerStyles}>
        <LinearGradient colors={[materialTheme.COLORS.PRIMARY, materialTheme.COLORS.PRIMARY]}>
          <NavBar
            back={back}
            title={title}
            style={styles.navbar}
